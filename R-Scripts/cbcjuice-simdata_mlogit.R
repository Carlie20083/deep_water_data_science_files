...
small.cbc <- mlogit.data(data=cbc.df[1:(25*15*3),], 
                         ...
                         varying=3:6, alt.levels=paste("pos", 1:3), 
                         id.var="resp.id")
m4 <- mlogit(choice ~ 0 + pulp + juice + added + price, data = small.cbc)
...
m1.rpar
m1.hier <- mlogit(choice ~ 0 + pulp + juice + added + price, 
                  data = cbc.mlogit, 
...
cbc.coded <- model.matrix(~ pulp + juice + added + price, data = cbc.df)
cbc.coded <- cbc.coded[, -1] # remove the intercept

choicemodelr.data <- cbind(cbc.df[, 1:3], cbc.coded, choice)
head(choicemodelr.data)

carpool <- cbc.df$healthy[cbc.df$ques==1 & cbc.df$alt==1]=="yes"
carpool <- as.numeric(healthy)
choicemodelr.demos <- as.matrix(healthy, nrow=length(healthy))
...
predict.hb.mnl <- function(betadraws, data) {
  ...
  # data: a data frame containing the set of designs for which you want to 
  # predict shares.  Same format at the data used to estimate model. 
  data.model <- model.matrix(~ pulp + juice + added + price, data = data)
